...
var allCarousels = document.getElementsByClassName('carousel-container'); // Iterate over all of the carousels using a FOR-loop

for (var i = 0; i < allCarousels.length; i++) {
  // Create a JSON object to keep track of each carousel
  var carousel = {
    container: allCarousels[i]
  }; // Get the <figure> element inside of this particular carousel

  carousel.figure = carousel.container.querySelector('figure'); // Grab the UL element inside of that

  carousel.figureCont = carousel.figure.querySelector('ul'); // Get all the list items inside of the UL

  carousel.items = carousel.figureCont.children; // Get the buttons, which will be used for manually changing the carousel
  ...
  carousel.currentItem = 0; // This is a CLOSURE function
...
function carouselNext(carousel) {
  ...
  carousel.currentItem++; // If we increased it too much
...
function carouselPrev(carousel) {
  ...
  carousel.currentItem--; // If we're below zero, we've gone too far
...
function displayCarouselItem(carousel) {
  ...
  startCarouselTimeout(carousel); // Set our new margin-left CSS property
...
function startCarouselTimeout(carousel) {
  ...
  clearTimeout(carousel.interval); // Create a new automatic timer
